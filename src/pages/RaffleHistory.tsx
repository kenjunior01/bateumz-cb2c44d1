      ...
      <div className="container mx-auto px-4 pt-28 pb-20">
        ...
        {loading ? (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-20">
            ...
            <h2 className="font-display text-xl font-bold text-foreground mb-2">Primeiros vencedores a caminho!</h2>
            <p className="text-muted-foreground mb-6 max-w-md mx-auto">
              Os sorteios activos estão a decorrer. Assim que o primeiro vencedor for seleccionado, aparecerá aqui com verificação blockchain.
            </p>
            ...
            {raffles.map((raffle, i) => (
              ...
              >
                <Link to={`/raffle/${raffle.slug || raffle.id}`}>
                  <Card className="glass group hover:border-accent/40 transition-all overflow-hidden">
                    <div className="relative aspect-[4/3] bg-secondary overflow-hidden">
                      ...
                      <Badge className="absolute top-3 left-3 bg-accent text-accent-foreground font-bold gap-1">
                        <Crown className="h-3 w-3" /> Concluído
                      </Badge>
                      {raffle.winner_ticket && (
                        <div className="absolute bottom-3 right-3">
                          <Badge className="bg-primary text-primary-foreground font-mono text-sm px-3">
                            #{raffle.winner_ticket}
                          </Badge>
                        </div>
                      )}
                    ...
                    <CardContent className="p-5">
                      ...
                      <p className="text-sm text-primary mb-3 line-clamp-1">{raffle.prize_title}</p>

                      {raffle.winner_name && (
                        <div className="flex items-center gap-2 rounded-xl bg-accent/10 border border-accent/20 p-3 mb-3">
