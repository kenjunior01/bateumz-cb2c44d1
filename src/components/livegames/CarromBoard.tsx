  ...
  useEffect(() => {
    ...
    if (justPocketed.length > 0) {
      ...
      if (turn === "player") {
        ...
        if (pts > 0) setMessage(`Voce encaçou ${pts} ponto(s)!`);
        else setMessage("Voce encaçou peça do adversario - turno passa!");
      } else {
        const pts = justPocketed.filter(c => c.color === botColor).length * 1 + justPocketed.filter(c => c.color === "queen").length * 2;
        setBotScore(b => b + pts);
        if (pts > 0) setMessage(`Bot encaçou ${pts} ponto(s)!`);
      }
    }
    ...
    <div className="w-full max-w-lg mx-auto">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-3">
          ...
          <div className="text-xs font-bold px-3 py-1 rounded-full bg-slate-200 text-slate-800 border border-slate-300">
            Bot (Preto): {botScore}
          </div>
        </div>
        <div className={"text-xs font-bold px-3 py-1 rounded-full " + (turn === "player" ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700")}>
      ...
      <motion.div
        ...
        style={{ background: boardBg, border: "6px solid #5C3A1E", boxShadow: "0 8px 32px rgba(0,0,0,0.4), inset 0 0 60px rgba(0,0,0,0.15)" }}
      ...
      >
        <svg viewBox={"0 0 " + BOARD_SIZE + " " + BOARD_SIZE} className="absolute inset-0 w-full h-full">
          ...
          {!striker.active && (
            <circle cx={striker.x} cy={striker.y} r={STRIKER_R + 2} fill="none" stroke="#F59E0B" strokeWidth="2" strokeDasharray="4 3" />
          )}
      ...
      <AnimatePresence mode="wait">
        <motion.p key={message} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="text-center text-xs text-muted-foreground mt-2 h-5">
          {message}
        </motion.p>
      </AnimatePresence>
      ...
      <AnimatePresence>
        {gameOver && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="mt-4 rounded-2xl bg-card border p-6 text-center">
            ...
            <p className="text-sm text-muted-foreground mb-4">Voce: {playerScore} | Bot: {botScore}</p>
            <Button onClick={restart} className="gap-2"><RotateCcw className="h-4 w-4" /> Jogar Novamente</Button>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
