        ...
        <div className="space-y-2">
          ...
          <div className="space-y-1.5">
            {data.options.map((opt: string, i: number) => {
              ...
              const isMyVote = hasVoted && votes[`_voter_${user?.id}`] === i;
              return (
                <button
                  key={i}
            ...
            <CardContent className="p-4">
              <div className="flex gap-2 mb-3 flex-wrap">
                {[
                  ...
                  <button key={t.id} onClick={() => { setMsgType(t.id); setShowPollCreator(false); }}
                    className={`rounded-full px-3 py-1 text-xs font-medium transition-all ${msgType === t.id && !showPollCreator ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground hover:text-foreground"}`}>
                    {t.label}
                  </button>
                ))}
                <button onClick={() => setShowPollCreator(!showPollCreator)}
                  className={`rounded-full px-3 py-1 text-xs font-medium transition-all ${showPollCreator ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground hover:text-foreground"}`}>
                  📊 Sondagem
                </button>
              </div>
                    ...
                    <div key={i} className="flex gap-2">
                      ...
                      {pollOptions.length > 2 && (
                        <button onClick={() => setPollOptions(pollOptions.filter((_, j) => j !== i))}
                          className="text-muted-foreground hover:text-destructive text-sm px-2">✕</button>
                      )}
                    </div>
                  ))}
